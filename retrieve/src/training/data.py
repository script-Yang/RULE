...
class RadiologyDataset(Dataset):  # TODO
    def __init__(
        ...
        for entry in self.data:
            ...
            self.image_report_pairs.append(
                    (os.path.join(img_root, entry["image_path"][0]), entry["report"])
                )
                # print(img_path)
                # exit(0)
...
class IUXrayVQADataset(Dataset):  # TODO
    def __init__(
        ...
        #         if "0.png" in img_path:
        #             self.image_report_pairs.append(
        #                 (os.path.join(img_root, img_path), entry["report"], entry["id"])
        #             )
        #         else:
        #             continue
                # self.image_report_pairs.append((img_path, entry["report"]))
    ...
    def __getitem__(self, idx):
        # img_path, report, id = self.image_report_pairs[idx]
        data_info = self.data[idx]
        img_path = os.path.join(self.img_root, data_info["image"])
        image = Image.open(img_path).convert("RGB")
        # data_info = {"id": id, "image": img_path, "report": report}
